...
class PrepRunNvNMDTrain(Steps):
    def __init__(
        self,
        name: str,
        prep_train_op: Type[OP],
        run_train_op: Type[RunNvNMDTrain],
...
def _prep_run_nvnmd_train(
    ...
    run_train = Step(
        ...
        artifacts={
            ...
            "init_model_ckpt_meta": train_steps.inputs.artifacts[
                "init_models_ckpt_meta"
            ],
            "init_model_ckpt_data": train_steps.inputs.artifacts[
                "init_models_ckpt_data"
            ],
            "init_model_ckpt_index": train_steps.inputs.artifacts[
    ...
    train_steps.outputs.artifacts["models"]._from = run_train.outputs.artifacts[
        "cnn_model"
    ]
    train_steps.outputs.artifacts["nvnmodels"]._from = run_train.outputs.artifacts[
        "qnn_model"
    ]
    train_steps.outputs.artifacts[
        "models_ckpt_meta"
    ]._from = run_train.outputs.artifacts["model_ckpt_meta"]
    train_steps.outputs.artifacts[
        "models_ckpt_data"
    ]._from = run_train.outputs.artifacts["model_ckpt_data"]
    train_steps.outputs.artifacts[
        "models_ckpt_index"
    ]._from = run_train.outputs.artifacts["model_ckpt_index"]
